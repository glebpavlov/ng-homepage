...
export class LoginComponent {
  ...
  isLoading$ = this.authFacade.isLoadingLogin$.pipe(
    ...
    tap(([prevValue, currentValue]) =>
      prevValue && !currentValue ? this.lastEndLoadingTime$.next(new Date()) : undefined
    ),
    map(([prevValue, currentValue]) => currentValue)
  );
  showLoginError$ = this.authFacade.hasLoginError$.pipe(
    switchMap(isError =>
      merge(of(isError), of(false).pipe(delay(this.errorMessageSeconds * 1000)))
    )
  );
